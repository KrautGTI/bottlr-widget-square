  ...
  (function () {
    ...
    style.textContent = `
      ...
      .bottlr-widget-button {
        ...
        background:linear-gradient(135deg,#00FF7F 0%,#00E066 100%); display:flex; align-items:center; justify-content:center;
        box-shadow:0 8px 25px rgba(0,255,127,.4); cursor:pointer; border:none; transition:all .3s ease; z-index:9999; animation:pulse 2s infinite;
      }
      @keyframes pulse {
      ...
      .bottlr-widget-card { width:100%; height:100%; display:flex; flex-direction:column; background:radial-gradient(50% 50% at 50% 50%, #99F3B4 0%, #fff 80%); box-shadow:-8px 0 24px rgba(0,0,0,.1); border-left:1px solid #d1fae5; }
    ...
    function ChatInterface() {
      ...
      useEffect(() => {
        try {
          ...
          setUserId(sid); setUserEmail(email); setChatId(`chat_${sid}`);
        } catch {
          const fid = `anonymous_${Date.now()}`;
          setUserId(fid); setUserEmail("anonymous@bottlr.local"); setChatId(`chat_${fid}`);
        }
      }, []);
      ...
      // Functional updater: either a string or a (prev) => newString
      const updateMessage = useCallback((id, updater) => {
        setMessages(prev =>
          prev.map(m =>
            m.id === id
              ? { ...m, content: (typeof updater === "function" ? updater(m.content || "") : updater) }
              : m
          )
        );
      }, []);
      ...
      useEffect(() => {
        ...
        // Optional: log everything during dev
        ...
        // Optional metadata channels
        socket.on("bttlr-stream-metadata", (data) => { /* stash if you want */ });
        socket.on("bttlr-workflow-metadata", (data) => { /* stash if you want */ });
        socket.on("bttlr-artifacts", (data) => { /* stash if you want */ });
        socket.on("bttlr-follow-up", (data) => { /* stash if you want */ });

        socket.on("bttlr-stream-end", (data) => {
          const id = streamingBotMsgIdRef.current;
          if (id && data?.final_response) {
            // keep streamed content; only fill if still empty
      ...
      const sendQuery = useCallback((chatId, userId, userEmail, query) => {
        ...
        socketRef.current.emit("bttlr-stream", { chat_id: chatId, user_id: userId, user_email: userEmail, query });
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          messages.map((msg) => {
            ...
            const isCurrentStreamingBot = msg.type === "bot" && msg.id === streamingBotMsgIdRef.current;
            const hasText = Boolean((responseData.textContent || "").trim());

            const contentEl = (isCurrentStreamingBot && !hasText && isTyping)
          ...
          // No separate "isTyping" bubble anymore (prevents duplicates)
          status && React.createElement("div", { className: "bottlr-status" }, status),
          progress > 0 &&
            React.createElement("div", { className: "bottlr-progress" },
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            isTyping
              ? React.createElement("button", { className: "bottlr-send-btn", title:"Streaming…", style:{ background:"#ef4444", borderRadius:"16px", opacity:.6, cursor:"not-allowed" } }, React.createElement(X, { size: 18, color: "#fff" }))
    ...
    // Same-domain link routing inside widget
    ...
    window.BottlrWidget = {
      ...
      version: "2.2.0-socket-final",
    };

    console.log("🍷 Bottlr Widget v2.2 (Socket.IO, fixed) loaded");
  })();
