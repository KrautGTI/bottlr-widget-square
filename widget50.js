...
(function () {
  ...
  console.log(
    __IS_MOBILE
      ...
      : "💻 Desktop device detected - Loading Bottlr widget"
  );
  /* ------- Wrapper / Containers ------- */
  ...
  widgetContainer.addEventListener("onmousemove", () => {
  ...
  .bottlr-product-image.is-loaded{ opacity:1; }
        ...
        /* Auth — compact & wrap */
        .bottlr-auth { display:flex; gap:8px; align-items:center; flex-wrap:wrap; max-width:100%; justify-content: center; margin: 5px;}
        ...
        .bottlr-userchip { display:flex; align-items:center; gap:8px; padding:6px 10px; border-radius:0px; background:white; font-size:12px; justify-content:center }
        ...
        @media (max-width:768px){
          ...
          .bottlr-auth input, .bottlr-auth button { width:120px; }
        }
      `;
  ...
  const getCartToken = () =>
    getCookie("com_cart_token") || getCookie("sq_cart_v1") || null;
  ...
  const isRenderableProduct = (p) =>
    hasVal(p?.name) &&
    (isInternalUrl(p?.url) || hasVal(p?.productId)) &&
    isHttpImage(p?.imageUrl);
  ...
  function parseTextAndProducts(input) {
    ...
    const textContent =
      firstTagIdx === -1 ? text.trim() : text.slice(0, firstTagIdx).trim();
    const products = [];
    ...
    for (const p of products) {
      const key =
        (p.url && `u:${p.url}`) ||
        (p.name && p.price && `np:${p.name}|${p.price}`) ||
        JSON.stringify(p);
      if (!seen.has(key)) {
        seen.add(key);
        deduped.push(p);
  ...
  function ProductCard({
    ...
    const navigateTo = useCallback((targetUrl) => {
      ...
      if (
        ...
        window.BottlrWidget &&
        typeof window.BottlrWidget.close === "function"
      ) {
  ...
  function ChatInterface({ chatId, userId, userEmail }) {
    const [messages, setMessages] = useState([
      ...
      },
      {
        ...
        type: "bot",
        content:
          "**We're actively developing this tool.** Interested in joining our beta testing round? Reach out to us at [bottlr@parlourwines.com](mailto:bottlr@parlourwines.com). v49",
      },
    ]);
    ...
    useEffect(() => {
      ...
      socket.on("bttlr-stream-start", () => {
        if (!streamingBotMsgIdRef.current)
          streamingBotMsgIdRef.current = addMessage("bot", "");
        setIsTyping(true);
      });
