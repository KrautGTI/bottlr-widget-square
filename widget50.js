 import React, { useState, useEffect, useCallback, useRef } from "https://esm.sh/react@18";
  ...
  import { MessageCircle, Send, X, Wine, Sparkles, User as UserIcon, Camera } from "https://esm.sh/lucide-react@0.263.0?bundle";
      ...
      link(href, title, text) {
        ...
        try {
          ...
          if (href && href.startsWith("http")) {
            ...
            if (linkUrl.hostname === currentDomain) processedHref = linkUrl.pathname + linkUrl.search + linkUrl.hash;
          }
        } catch (e) {}
        let linkClass = "";
  ...
  (function() {
    ...
    Object.assign(wrapper.style, { display:"flex", width:"100vw", height:"100dvh", margin:"0", padding:"0" });

    const siteContainer = document.createElement("div");
    siteContainer.id = "bottlr-site-container";
    Object.assign(siteContainer.style, { flex:"1 1 100%", height:"100dvh", overflow:"auto", boxSizing:"border-box" });

    const widgetContainer = document.createElement("div");
    widgetContainer.id = "bottlr-widget-container";
    Object.assign(widgetContainer.style, { flex:"0 0 0", height:"100dvh", boxSizing:"border-box", overflow:"hidden" });
    ...
    style.textContent = `
      ...
      .bottlr-widget-button { position:fixed; bottom:20px; right:20px; width:60px; height:60px; border-radius:50%;
        background:linear-gradient(135deg,#00FF7F 0%,#00E066 100%); display:flex; align-items:center; justify-content:center; cursor:pointer; border:none; transition:all .3s; z-index:9999; }
      ...
      .bottlr-img-bubble { margin-top:8px; border-radius:12px; overflow:hidden; }
      .bottlr-img-bubble img { display:block; max-width:220px; height:auto; }
      .bottlr-pending-image { display:flex; align-items:center; gap:8px; background:#fff; border:1px dashed #c7c7c7; padding:8px; border-radius:10px; margin:8px 0 0; }
      .bottlr-pending-image img { width:56px; height:56px; object-fit:cover; border-radius:8px; }
      .bottlr-pending-image button { border:none; background:#111; color:#fff; border-radius:8px; padding:6px 10px; cursor:pointer; }
      .bottlr-product-carousel { display:flex; gap:12px; overflow-x:auto; padding:6px 2px; scroll-snap-type:x mandatory; -webkit-overflow-scrolling:touch; }
      ...
      .bottlr-product-image-wrap{ width:100%; display:grid; place-items:center; overflow:hidden; border-radius:8px; margin:0 auto 16px; }
      .bottlr-product-image{ width:auto; height:auto; max-width:90%; max-height:90%; display:block; border-radius:8px; opacity:0; transition:opacity .25s ease; }
    ...
    async function registerUser(email, password) {
      const { data } = await axios.post(`${API_BASE}/auth/register`, { email, password }, { headers: { "Content-Type": "application/json" }});
      setTokens(data.tokens);
      return data;
    }
    async function loginUser(email, password) {
      const { data } = await axios.post(`${API_BASE}/auth/login`, { email, password }, { headers: { "Content-Type": "application/json" }});
      setTokens(data.tokens);
      return data;
    }
    ...
    function getCookie(name) {
      ...
      if (!s) return null;
      const p = name + "=";
      for (const part of s.split("; ")) if (part.startsWith(p)) return decodeURIComponent(part.slice(p.length));
      return null;
    }
    ...
    async function readCartV2() { return commerceV2("Cart::read", [false]); }
    ...
    const isInternalUrl = (s) => {
      ...
      try { return new URL(v).hostname === window.location.hostname; } catch (e) { return false; }
    };
    const isRenderableProduct = (p) => hasVal(p?.name) && (isInternalUrl(p?.url) || hasVal(p?.productId)) && isHttpImage(p?.imageUrl);
    ...
    function parseTextAndProducts(input) {
      ...
      for (const block of blocks) {
        ...
        const p = { category:null, name:null, price:null, imageUrl:null, url:null, productId:null, quantity:1, paramsOverride:null };
        for (const line of lines) {
          ...
          if (!hasVal(v)) v = null;
          switch (k) {
            case "CATEGORY": p.category = v; break;
            case "NAME":     p.name = v; break;
            case "PRICE":    p.price = v; break;
            case "URL":      p.url = v; break;
            case "PRODUCT_ID":
            case "SITE_PRODUCT_ID": p.productId = v; break;
            case "PARAMS":
              if (v) {
                try {
              ...
              break;
            case "QTY":
            case "QUANTITY": p.quantity = Number(v) || 1; break;
            case "IMAGE":
              if (v) {
                const md = v.match(/\[([^\]]+)\]\(([^)]+)\)/);
      ...
      return { textContent, products: deduped };
    }

    /* =================== Image helpers (mobile capture) =================== */
    ...
    async function downscaleDataURL(dataURL, maxSize = 1280, quality = 0.85) {
      return new Promise((resolve, reject) => {
        const img = new Image();
        img.onload = () => {
          let { width, height } = img;
          if (width <= maxSize && height <= maxSize) return resolve(dataURL);
    ...
    function clampDataURLSize(dataURL, maxBytes = 2500000) {
      if (!dataURL) return dataURL;
      const approxBytes = Math.floor(dataURL.length * 0.75);
      return approxBytes <= maxBytes ? dataURL : dataURL;
    }

    /* =================== UI bits =================== */
    function ProductCard({ category, imageUrl, name, price, url, buttonText = "Add to Cart", productId, quantity = 1, paramsOverride = null }) {
      ...
      const handleImgError = () => { setImgError(true); console.warn("[Bottlr] Product image failed to load:", imageUrl); };
      ...
      const navigateTo = useCallback((targetUrl) => {
        ...
        if (window.innerWidth <= 768 && window.BottlrWidget && typeof window.BottlrWidget.close === "function") window.BottlrWidget.close();
      }, []);

      const handleAddToCart = useCallback(async () => {
        if (!productId && url) { navigateTo(url); return; }
        if (!productId) {
          setLabel("Missing ID");
      ...
      return React.createElement(
        ...
        React.createElement("div", { className:"bottlr-product-image-wrap", "data-has-src": !!imageUrl },
          (imageUrl && !imgError)
            ? React.createElement("img", {
                className: `bottlr-product-image ${imgLoaded ? "is-loaded" : ""}`,
                src: imageUrl, alt: name || "Product",
                loading: (window.innerWidth <= 768) ? "eager" : "lazy",
                decoding: "async",
                onLoad: handleImgLoad,
                onError: handleImgError
              })
            : React.createElement("div", { style:{ display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, color:"#777", width:"100%", height:"80px" }}, "No image")
        ),
        name && React.createElement("div", { className:"bottlr-product-name" }, name),
        React.createElement("button", { className:"bottlr-product-button", onClick: () => navigateTo(url), disabled: busy },
          `${label}${price ? ` ${price}` : ""}`)
      );
    }
    ...
    function ChatInterface({ chatId, userId, userEmail }) {
      const [messages, setMessages] = useState([
        { id:"welcome", type:"bot", content:"Hey! Let's find your perfect wine. How do you want to start?", options:["Use my purchase history","Ask the AI-Sommelier","Find wines for dinner pairing","Get recommendations by style"] },
        { id:"disclaimer", type:"bot", content:"**We're actively developing this tool.** Interested in joining our beta testing round? Reach out to us at [bottlr@parlourwines.com](mailto:bottlr@parlourwines.com). v50" },
      ]);
      ...
      useEffect(() => { scrollToBottom(); }, [messages, isTyping, pendingImage, scrollToBottom]);

      const addMessage = useCallback((type, content, options = null, extras = {}) => {
        const m = { id: String(Date.now()) + Math.random().toString(36).slice(2,11), type, content, options, timestamp: new Date(), ...extras };
        setMessages(prev => [...prev, m]);
        return m.id;
      }, []);
      const updateMessage = useCallback((id, updater) => {
        setMessages(prev => prev.map(m => (m.id === id ? { ...m, content: (typeof updater === "function" ? updater(m.content || "") : updater) } : m)));
      }, []);

      // Socket
      useEffect(() => {
        const socket = io(SOCKET_URL, { path: SOCKET_PATH, transports:["websocket"], auth:{ token: getAccessToken() || "" } });
        socketRef.current = socket;

        socket.onAny((event, ...args) => {
          try { console.log("[Bottlr][SOCKET][EVENT]", event, JSON.stringify(args?.[0] ?? null)); }
          catch (e) { console.log("[Bottlr][SOCKET][EVENT]", event, args); }
        });
        ...
        const armWatchdog = () => {
          ...
          streamWatchdog = setTimeout(() => {
            ...
            if (id) {
              updateMessage(id, (prev) => (prev || "") + `\n\n_(No data received — check server logs / LLM errors.)_`);
              setIsTyping(false); setStatus(""); setProgress(0); streamingBotMsgIdRef.current = null;
            }
          }, 20000);
        };

        // ---- Streaming Handlers (robust to payload shapes) ----
        ...
        socket.on("bttlr-stream-chunk", (payload) => {
          const id = streamingBotMsgIdRef.current;
          if (!id) return;
          clearTimeout(streamWatchdog);
          armWatchdog();
          const piece =
            (payload && (payload.chunk ?? payload.delta ?? payload.text)) ??
            (typeof payload === "string" ? payload : "");
          if (!piece) return;
          updateMessage(id, prev => (prev || "") + piece);
        });

        socket.on("bttlr-stream-end", (payload) => {
          clearTimeout(streamWatchdog);
          const id = streamingBotMsgIdRef.current;
          const finalText =
            (payload && (payload.final_response ?? payload.data ?? payload.text ?? payload.message)) ?? "";
          if (id) {
            if (finalText) updateMessage(id, prev => prev || finalText);
          } else if (finalText) {
        ...
        socket.on("bttlr-stream-error", (payload) => {
          clearTimeout(streamWatchdog);
          const id = streamingBotMsgIdRef.current;
          const errMsg = (payload && (payload.error ?? payload.message)) || "Unknown error";
          if (id) updateMessage(id, prev => (prev || "") + `\n\nError: ${errMsg}`);
          else addMessage("bot", `Error: ${errMsg}`);
          setIsTyping(false); setStatus(""); setProgress(0);
          streamingBotMsgIdRef.current = null;
        });
      ...
      // Emit with image + token
      const sendQuery = useCallback((cid, uid, uemail, query, imageDataUrl = null) => {
        ...
        socketRef.current.emit("bttlr-stream", payload, (ack) => {
          if (ack && ack.error) {
            const id = streamingBotMsgIdRef.current;
            if (id) updateMessage(id, (prev) => (prev || "") + `\n\nError: ${ack.error}`);
            else addMessage("bot", `Error: ${ack.error}`);
            setIsTyping(false); setStatus(""); setProgress(0); streamingBotMsgIdRef.current = null;
          }
        });
      }, [socketState.connected, addMessage, updateMessage]);

      // User actions
      const handleSendMessage = useCallback((msg) => {
        const t = (msg || "").trim();
        if (!t && !pendingImage) return; // allow image-only sends
        if (isTyping) return;
        const extras = pendingImage ? { image: pendingImage } : {};
        addMessage("user", t || "(Image)", null, extras);
        setInput("");
        ...
        sendQuery(cid, uid, uem, t, pendingImage);
        setPendingImage(null);
      }, [isTyping, addMessage, sendQuery, chatId, userId, userEmail, pendingImage]);

      const handleOptionClick   = useCallback((o) => handleSendMessage(o), [handleSendMessage]);
      const handleInputKeyDown  = useCallback((e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleSendMessage(input); } }, [input, handleSendMessage]);
      const handleInputChange   = useCallback((e) => { const el = e.target; setInput(el.value); el.style.height = "auto"; el.style.height = Math.min(el.scrollHeight, 120) + "px"; }, []);
      const handlePickImage     = useCallback(() => { if (fileInputRef.current) fileInputRef.current.click(); }, []);
      const handleImageChosen   = useCallback(async (e) => {
        const file = e.target.files && e.target.files[0];
        e.target.value = "";
        if (!file) return;
        if (file.size > 10 * 1024 * 1024) { addMessage("bot", "Image too large (max 10MB). Try a smaller photo."); return; }
        try {
          const raw   = await fileToDataURL(file);
          const small = await downscaleDataURL(raw, 1280, 0.85);
          const safe  = clampDataURLSize(small, 2500000);
          setPendingImage(safe);
        } catch (err) {
          console.error("[Bottlr][Camera] read error", err);
          addMessage("bot", "Couldn't read that image. Try again.");
        }
      }, [addMessage]);
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          messages.map((msg) => {
            const parsed = msg.type === "bot" ? parseTextAndProducts(msg.content) : { textContent: msg.content, products: [] };
            ...
            const contentEl = (isStreaming && !hasText && isTyping)
              ...
              : React.createElement("div", { className:"bottlr-msg-content", dangerouslySetInnerHTML:{ __html: marked.parse(parsed.textContent || "") }});
            ...
            return React.createElement(
              ...
              (msg.type === "user" && msg.image) &&
                React.createElement("div", { className:"bottlr-img-bubble" },
                  React.createElement("img", { src: msg.image, alt:"uploaded", loading:"eager", decoding:"async" })
                ),
              products.length > 0 && (
                products.length === 1
                  ...
                  : React.createElement("div", { className:"bottlr-product-carousel" },
                      products.map((p, idx) => React.createElement(ProductCard, {
                        key: `${msg.id}-p${idx}`,
                        category: p.category, imageUrl: p.imageUrl, name: p.name, price: p.price, url: p.url,
                        productId: p.productId, quantity: p.quantity || 1, paramsOverride: p.paramsOverride || null
                      }))
                    )
              ),
              msg.options && React.createElement("div", { className:"bottlr-options" },
                msg.options.map((opt, i) =>
                  React.createElement("button", { key:i, className:"bottlr-option-btn", onClick: () => handleOptionClick(opt) },
                    React.createElement(Wine, { size:18, color:"#fff" }), opt
                  )
                )
              )
            );
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            __IS_MOBILE && React.createElement("button", { className:"bottlr-send-btn", title:"Add photo", onClick: handlePickImage, "aria-label":"Add photo", disabled: isTyping }, React.createElement(Camera, { size:18 })),
            React.createElement("textarea", {
              className:"bottlr-input",
              placeholder: isTyping ? "Bottlr is thinking..." : (__IS_MOBILE ? "Type a message… (or add a photo)" : "Ask me about wine..."),
              value: input, onChange: handleInputChange, onKeyDown: handleInputKeyDown, rows:1, disabled: isTyping
            }),
            isTyping
              ? React.createElement("button", { className:"bottlr-send-btn", title:"Streaming…", style:{ background:"#ef4444", borderRadius:"14px", opacity:.6, cursor:"not-allowed" }, disabled:true }, React.createElement(X, { size:18, color:"#fff" }))
              : React.createElement("button", { className:"bottlr-send-btn", onClick: () => handleSendMessage(input), disabled: (!input.trim() && !pendingImage) || isTyping, title:"Send" }, React.createElement(Send, { size:18, color:"#000" }))
          ),
          pendingImage && React.createElement("div", { className:"bottlr-pending-image" },
            React.createElement("img", { src: pendingImage, alt:"preview" }),
            React.createElement("span", null, "Photo ready to send"),
            React.createElement("button", { onClick: removePendingImage }, "Remove")
          ),
          __IS_MOBILE && React.createElement("input", { ref:fileInputRef, type:"file", accept:"image/*", capture:"environment", style:{ display:"none" }, onChange: handleImageChosen })
        )
      );
    }
    ...
    function ChatWidget() {
      ...
      useEffect(() => {
        ...
        history.pushState = function(...a){ p.apply(this, a); upd(); };
        history.replaceState = function(...a){ r.apply(this, a); upd(); };
        return () => {
      ...
      useEffect(() => {
        (async () => {
          ...
          try {
            ...
            setUserId(me.id); setEmail(me.email);
      ...
      const handleLogout = () => {
        ...
        localStorage.removeItem("bottlr_email");
        localStorage.removeItem("bottlr_chat_id");
        setUserId(null); setEmail(null); setChatId(null);
      };
      ...
      useEffect(() => {
        window.BottlrWidget = {
          ...
          version: "3.0.0-mobile-image-stream",
          readCart: readCartV2,
      ...
      return React.createElement(
        ...
        isOpen && React.createElement("div", { className:"bottlr-widget-card" },
          React.createElement("div", { className:"bottlr-widget-header" },
            React.createElement("h3", null, React.createElement(Wine, { size:18 }), "Bottlr"),
            React.createElement(AuthPanel, { userId, userEmail, onAuthed: handleAuthed, onLogout: handleLogout, busy: authBusy }),
            React.createElement("button", { className:"bottlr-close-btn", onClick: toggleOpen, "aria-label":"Close chat" }, React.createElement(X, { size:18 }))
          ),
          React.createElement("div", { className:"bottlr-current-url", title: currentURL }, currentURL),
          React.createElement("div", { style:{ padding:"0 12px 8px", fontSize:"12px", color:"#333", display:"flex", alignItems:"center", gap:6 } },
          ...
          React.createElement("div", { style:{ padding:"0 0 10px 0", textAlign:"center", fontSize:"13px", color:"#000", background:"rgba(187,237,200,.40)", zIndex:10 } }, "Powered by Bottlr")
        )
      );
    }
    ...
    function AuthPanel({ userId, userEmail, onAuthed, onLogout, busy }) {
      ...
      if (userId && userEmail) {
        return React.createElement("div", { className:"bottlr-auth" },
      ...
      return React.createElement("div", { className:"bottlr-auth" },
        React.createElement("input",  { type:"email",    placeholder:"email",    value: email,    onChange: e => setEmail(e.target.value),    disabled: busy }),
        React.createElement("input",  { type:"password", placeholder:"password", value: password, onChange: e => setPassword(e.target.value), disabled: busy }),
        React.createElement("button", { className:"btn-ghost",   onClick: handleLogin,    disabled: busy || !email || !password }, "Login"),
        React.createElement("button", { className:"btn-primary", onClick: handleRegister, disabled: busy || !email || !password }, "Register"),
