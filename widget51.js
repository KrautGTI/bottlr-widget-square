import React, {
	...
	/* =================== STRING SAFETY =================== */
	// Safely coerce any value into a string for rendering/parsing
	function asString(v) {
    ...
    `;
		...
		function parseTextAndProducts(input) {
			const text = asString(input); // <= safe
		...
		function ChatInterface({
			...
			useEffect(() => {
				...
				socket.on("bttlr-stream-start", (data) => {
					...
					// If server sends an initial chunk in start:
				...
				socket.on("bttlr-stream-end", (data) => {
					...
					const id = streamingBotMsgIdRef.current;
					if (id) {
						const finalText = asString(
							data?.final_response ?? data?.data ?? data?.message ?? data?.text ?? data?.content ?? ""
						);
			...
			return React.createElement(
				...
				React.createElement(
					...
					messages.map((msg) => {
						const parsed = msg.type === "bot" ?
							parseTextAndProducts(msg.content) :
							{
				...
				React.createElement(
					...
					React.createElement(
						...
						__IS_MOBILE && React.createElement(
							"button", {
