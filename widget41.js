import React, {
  ...
  return null;
		...
		async function commerceV2(method, params, id = 0) {
			...
			if (!xsrf) throw new Error('Missing XSRF-TOKEN cookie.');
		...
		async function addToCartV2(productId, quantity = 1, extraParams = null) {
			// If caller passed a full params array (site-specific), use it. Else use [id, qty].
		...
		function ProductCard({
			...
			const handleAddToCart = useCallback(async () => {
				// If we don't have a productId, fall back to navigation.
				if (!productId && url) {
					let targetPath = url;
					try {
						const currentDomain = window.location.hostname;
				...
				if (!productId) {
					setLabel("Missing ID");
					setTimeout(() => setLabel(buttonText), 1600);
					return;
				}
				...
				try {
					...
					// Dispatch a site-wide event others can listen for (mini-cart, badges, etc.)
			...
			}, [productId, quantity, paramsOverride, url, buttonText]);
