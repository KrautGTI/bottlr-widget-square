import React, {
    ...
    .bottlr-pending-image {
      display:flex; align-items:center; gap:8px; background:#fff; border:1px dashed #c7c7c7;
      padding:8px; border-radius:10px; margin:8px 0 0;
    }
  ...
  `;
		...
		// NEW: Normalize to a data URL, in case something upstream passes raw base64
		function normalizeAsDataURL(value, fallbackMime = "image/jpeg") {
			if (!value) return null;
			const v = String(value).trim();
			if (v.startsWith("data:")) return v;
			// If it's already like "UklGRkgZ..." etc., add header
			return `data:${fallbackMime};base64,${v}`;
		}
		/* =================== UI bits =================== */
		function ProductCard({
			...
			const [label] = useState(buttonText);
		...
		function ChatInterface({
			...
			const [messages, setMessages] = useState([{
				...
				},
				{
					id: "disclaimer",
					type: "bot",
					content: "**We're actively developing this tool.** Interested in joining our beta testing round? Reach out to us at [bottlr@parlourwines.com](mailto:bottlr@parlourwines.com). v50"
				},
			]);
			...
			useEffect(() => {
				scrollToBottom();
			}, [messages, isTyping, pendingImage, scrollToBottom]);
			...
			const updateMessage = useCallback((id, updater) => {
				setMessages(prev => prev.map(m => (m.id === id ? {
					...m,
					content: (typeof updater === "function" ? updater(asString(m.content)) : updater)
				} : m)));
			}, []);
			// Socket
			useEffect(() => {
				...
				socket.onAny((event, ...args) => {
					try {
						console.log("[Bottlr][SOCKET][EVENT]", event, JSON.stringify(args?.[0] ?? null));
					} catch {
						console.log("[Bottlr][SOCKET][EVENT]", event, args);
					}
				});
				...
				socket.on("bttlr-stream-start", (data) => {
					...
					const seed = asString(data?.chunk ?? data?.data ?? data?.message ?? data?.text ?? data?.content ?? "");
					if (seed) updateMessage(streamingBotMsgIdRef.current, (prev) => asString(prev) + seed);
					setIsTyping(true);
					setStatus("Streaming…");
					armWatchdog();
				});
				socket.on("bttlr-status-update", (data) => setStatus(asString(data?.status || "")));
				...
				socket.on("bttlr-stream-chunk", (data) => {
					...
					clearTimeout(streamWatchdog);
					armWatchdog();
					const chunk = asString(data?.chunk ?? data?.data ?? data?.message ?? data?.text ?? data?.content ?? "");
					if (chunk) updateMessage(id, prev => asString(prev) + chunk);
				});
				socket.on("bttlr-stream-end", (data) => {
					clearTimeout(streamWatchdog);
					const id = streamingBotMsgIdRef.current;
					if (id) {
						const finalText = asString(data?.final_response ?? data?.data ?? data?.message ?? data?.text ?? data?.content ?? "");
						if (finalText) updateMessage(id, prev => asString(prev) || finalText);
					}
					setIsTyping(false);
					setStatus("");
					setProgress(0);
				...
				socket.on("bttlr-stream-error", (data) => {
					clearTimeout(streamWatchdog);
					const id = streamingBotMsgIdRef.current;
					if (id) updateMessage(id, prev => asString(prev) + `\n\nError: ${asString(data?.error || "Unknown error")}`);
					setIsTyping(false);
					setStatus("");
					setProgress(0);
					streamingBotMsgIdRef.current = null;
			...
			// Emit with image + token — FIXED payload for backend
			const sendQuery = useCallback((cid, uid, uemail, query, imageDataUrl = null) => {
				...
				// FULL data URL — no truncation, no preview field
				const normalizedImage = imageDataUrl ? normalizeAsDataURL(imageDataUrl) : null;
				const payload = {
					chat_id: cid,
					user_id: uid,
					user_email: uemail,
			...
			const handleSendMessage = useCallback((msg) => {
				const t = asString(msg).trim();
				if (!t && !pendingImage) return; // allow image-only sends
				if (isTyping) return;
				const extras = pendingImage ? {
					image: pendingImage
				} : {};
			...
			// Mobile: trigger camera/file picker
			...
			const handleImageChosen = useCallback(async (e) => {
				...
				if (file.size > 10 * 1024 * 1024) {
					addMessage("bot", "Image too large (max 10MB). Try a smaller photo.");
					return;
				}
				try {
					...
					// ensure header is correct even if browser quirks
					const normalized = normalizeAsDataURL(safe, file.type || "image/jpeg");
			...
			return React.createElement(
				...
				React.createElement("div", {
					...
					messages.map((msg) => {
						const parsed = msg.type === "bot" ? parseTextAndProducts(msg.content) : {
							textContent: asString(msg.content),
							products: []
						};
						const isStreaming = msg.type === "bot" && msg.id === streamingBotMsgIdRef.current;
						const textStr = asString(parsed.textContent);
				...
				React.createElement("div", {
					...
					React.createElement("div", {
						...
						__IS_MOBILE && React.createElement("button", {
						...
						React.createElement("button", {
							...
							disabled: (!asString(input).trim() && !pendingImage) || isTyping,
							title: "Send"
					...
					pendingImage && React.createElement("div", {
							className: "bottlr-pending-image"
						},
						React.createElement("img", {
							src: pendingImage,
							alt: "preview"
					...
					__IS_MOBILE && React.createElement("input", {
						ref: fileInputRef,
						type: "file",
						accept: "image/*",
						capture: "environment",
