import React, {
	...
	(function() {
      ...
      /* Auth Panel */
      .bottlr-auth { display:flex; gap:8px; align-items:center; }
      .bottlr-auth input {
    ...
    `;
		...
		async function addToCartV2(productId, quantity = 1, extraParams = null) {
			...
			console.log("[Bottlr][Cart] addToCartV2", {
				productId,
				quantity,
				params
			});
		...
		// ===== 7) Auth Panel (updated to use new auth shape) =====
		function AuthPanel({
			userId,
			userEmail,
			onAuthed,
		...
		// ===== 8) Chat Interface (socket streaming; now sends token on connect) =====
		function ChatInterface({
			...
			const [messages, setMessages] = useState([{
				...
				},
				{
					id: "disclaimer",
					type: "bot",
					content: "**We're actively developing this tool.** Interested in joining our beta testing round? Reach out to us at [bottlr@parlourwines.com](mailto:bottlr@parlourwines.com). v42",
				},
			]);
		...
		window.BottlrWidget = {
			...
			version: "2.5.0-socket-auth-jwt",
			readCart: readCartV2,
			addToCart: addToCartV2
		};
		console.log("🍷 Bottlr Widget loaded with JWT auth & chat bootstrap.");
	})();
